  ...
  while (1)
  {
    ...
    /* USER CODE BEGIN 3 */
	  if( 1 == ButtonInterruptFlag )
	  {
         ...
         while(1 != AdcConvertCpltFlag){}

		    if( AdcValue >= 0 &&  AdcValue < 2000)
		    {
		    ...
		    else if( AdcValue >= 3000 &&  AdcValue < 4000)
		    {
				  HAL_GPIO_WritePin(GPIOD,  LED_ORANGE_Pin, GPIO_PIN_SET);
				  HAL_GPIO_WritePin(GPIOD,  LED_RED_Pin, GPIO_PIN_SET);
...
static void CAN1_Tx(void)
{
    ...
    TxHeader.StdId = Board ;
    TxHeader.RTR= CAN_RTR_DATA;
	TxHeader.IDE= CAN_ID_STD;
	TxHeader.DLC= 1;
	TxHeader.TransmitGlobalTime = DISABLE;
	if(HAL_CAN_AddTxMessage(&hcan1, &TxHeader,CanMsg , &TxMailbox) != HAL_OK)
	{
		Error_Handler();
	}
}
